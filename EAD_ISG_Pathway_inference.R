library(dplyr)
library(Seurat)
library(SeuratObject)
library(sctransform)
library(ggplot2)
library(ggpubr)
library(NMF)
library(remotes)
library(SeuratDisk)
library(Matrix)
library(openxlsx)
library(Nebulosa)
library(HGNChelper)
library(scCustomize)
library(AnnotationDbi)
library(CellChat)
library(scRNAtoolVis)
library(cowplot)
library(pheatmap)
library(dittoSeq)
library(genekitr)
library(enrichplot)
library(SCP)
library(decoupleR)
library(readr)


##########################EAD analysis########################################
sc_data <- Seurat::Read10X("SC-CountData", gene.column = 1)
sc_data <- CreateSeuratObject(counts = sc_data, min.cells = 3, min.features = 200)
SC_metaData <- read.csv("SC-MetaData.csv")
rownames(SC_metaData) <- SC_metaData$cell_id
sc_data <- AddMetaData(object = sc_data, metadata = SC_metaData)

sc_data[["percent.mt"]] <- PercentageFeatureSet(sc_data, pattern = "^MT-")
sc_data[["percent.hb"]] <- PercentageFeatureSet(sc_data, pattern = "^HB")
sc_data[["percent.ribo"]] <- PercentageFeatureSet(sc_data, pattern = "^RP[SL]")
sc_data <- subset(sc_data, subset = nFeature_RNA > 200 & nFeature_RNA < 4000 & percent.mt < 50)
sc_data <- SCTransform(sc_data, vars.to.regress = c("percent.mt", "percent.ribo", "percent.hb"), verbose = TRUE)
sc_data <- RunPCA(sc_data, verbose = FALSE)
sc_data <- RunUMAP(sc_data, dims = 1:50, verbose = FALSE)
sc_data <- FindNeighbors(sc_data, dims = 1:50, verbose = FALSE)
sc_data <- FindClusters(sc_data, verbose = FALSE)

CellDimPlot(sc_data, group.by = c("ead", "celltype"), reduction = "UMAP", theme_use = "theme_blank")

ISG_list <- c("ADAR", "B2M", "BATF2", "BST2", "C1S", "CASP1", "CASP8", "CCRL2", "CD47", "CD74", 
              "CMPK2", "CNP", "CSF1", "CXCL10", "CXCL11", "DDX60", "DHX58", "EIF2AK2", "ELF1", "EPSTI1", 
              "MVB12A", "TENT5A", "CMTR1", "GBP2", "GBP4", "GMPR", "HERC6", "HLA-C", "IFI27", "IFI30", 
              "IFI35", "IFI44", "IFI44L", "IFIH1", "IFIT2", "IFIT3", "IFITM1", "IFITM2", "IFITM3", "IL15", 
              "IL4R", "IL7", "IRF1", "IRF2", "IRF7", "IRF9", "ISG15", "ISG20", "LAMP3", "LAP3", "LGALS3BP", 
              "LPAR6", "LY6E", "MOV10", "MX1", "NCOA7", "NMI", "NUB1", "OAS1", "OASL", "OGFR", "PARP12", "PARP14", 
              "PARP9", "PLSCR1", "PNPT1", "HELZ2", "PROCR", "PSMA3", "PSMB8", "PSMB9", "PSME1", "PSME2", 
              "RIPK2", "RNF31", "RSAD2", "RTP4", "SAMD9", "SAMD9L", "SELL", "SLC25A28", "SP110", "STAT2", 
              "TAP1", "TDRD7", "TMEM140", "TRAFD1", "TRIM14", "TRIM21", "TRIM25", "TRIM26", "TRIM5", "TXNIP", 
              "UBA7", "UBE2L6", "USP18", "WARS1", "APOL6", "ARID5B", "ARL4A", "AUTS2", "BANK1", "BPGM", "BTG1", 
              "C1R", "CASP3", "CASP4", "CASP7", "CCL2", "CCL5", "CCL7", "CD274", "CD38", "CD40", "CD69", "CD86", 
              "CDKN1A", "CFB", "CFH", "CIITA", "CMKLR1", "CSF2RB", "CXCL9", "DDX58", "EIF4E3", "FAS", "FCGR1A", 
              "FGL2", "FPR1", "GBP6", "GCH1", "GPR18", "GZMA", "HIF1A", "HLA-A", "HLA-B", "HLA-DMA", "HLA-DQA1", 
              "HLA-DRB1", "HLA-G", "ICAM1", "IDO1", "IFIT1", "IFNAR2", "IL10RA", "IL15RA", "IL18BP", "IL2RB", "IL6", 
              "IRF4", "IRF5", "IRF8", "ISOC1", "ITGB7", "JAK2", "KLRK1", "LATS2", "LCP2", "LYSMD2", "MARCH1", 
              "METTL7B", "MT2A", "MTHFD2", "MVP", "MX2", "MYD88", "NAMPT", "NCOA3", "NFKB1", "NFKBIA", 
              "NLRC5", "NOD1", "NUP93", "OAS2", "OAS3", "P2RY14", "PDE4B", "PELI1", "PFKP", "PIM1", 
              "PLA2G4A", "PML", "PNP", "PSMA2", "PSMB10", "PSMB2", "PTGS2", "PTPN1", "PTPN2", "PTPN6", 
              "RAPGEF6", "RBCK1", "RIPK1", "RNF213", "SAMHD1", "SECTM1", "SELP", "SERPING1", 
              "SLAMF7", "SOCS1", "SOCS3", "SOD2", "SPPL2A", "SRI", "SSPN", "ST3GAL5", "ST8SIA4", "STAT1", "STAT3", 
              "STAT4", "TAPBP", "TNFAIP2", "TNFAIP3", "TNFAIP6", "TNFSF10", "TOR1B", "UPP1", "VAMP5", "VAMP8", 
              "VCAM1", "XAF1", "XCL1", "ZBP1", "ZNFX1", "WARS")


filter_ISG_list <- function(sc_data, ISG_list) {
  assay_data_genes <- rownames(GetAssayData(sc_data, assay = "SCT", slot = "data"))
  return(ISG_list[ISG_list %in% assay_data_genes])
}

ISG_list <- filter_ISG_list(sc_data, ISG_list)
ISG_scores <- Matrix::colSums(GetAssayData(sc_data, assay = "SCT", slot = "data")[ISG_list, ])
sc_data <- AddMetaData(sc_data, metadata = ISG_scores, col.name = "ISG_scores")
p1 <- CellDimPlot(sc_data, group.by = "celltype", split.by = "ead")
p2 <-FeatureDimPlot(sc_data, features = "ISG_scores", split.by = "ead")
p3 <- FeatureStatPlot(sc_data, stat.by = "ISG_scores", split.by = "ead", comparisons = TRUE)
p1/p2/p3

cp <- subset(sc_data, condition %in% "CP")
pp <- subset(sc_data, condition %in% "PP")
CellDimPlot(cp, group.by = "celltype", split.by = "ead")
FeatureDimPlot(cp, features = "ISG_scores", split.by = "ead")
FeatureDimPlot(pp, features = "ISG_scores", split.by = "ead")
FeatureStatPlot(cp, stat.by = "ISG_scores", split.by = "ead", comparisons = TRUE)
FeatureStatPlot(pp, stat.by = "ISG_scores", split.by = "ead", comparisons = TRUE)

###################################pathway activity inference analysis#############
Idents(sc_data) <- "ead"
sample <- sc_data
assay <- "SCT"

network <- decoupleR::get_progeny(organism = "human")
activities <- decoupleR::run_wmean(mat = as.matrix(sample@assays[[assay]]@data),
                                   network = network,
                                   .source = "source",
                                   .targe = "target",
                                   .mor = "weight",
                                   times = 100,
                                   minsize = 5)

out <- SCpubr::do_PathwayActivityPlot(sample = sample,
                                      activities = activities)
out
